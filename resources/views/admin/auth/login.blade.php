<x-admin-guest-layout>
    <x-auth-card>
        ...
        <form x-on:submit="buttonDisabled = true" method="POST" action="{{ route('admin.login') }}">
            @csrf
            ...
            <!-- Password -->
                ...
                <div x-data="{showPassword: false}" class="relative">
                    ...
                    <x-text-input id="password" class="mt-1 w-full relative pr-12"
            ...
            <div class="flex flex-col mx-auto items-left lg:items-center justify-between mt-2 md:mt-6 mb-2 space-y-6 lg:flex-row lg:space-y-0">
                @if (Route::has('admin.password.request'))
                    <a @click="buttonDisabled = true" class="font-thin sm:mr-72 md:ml-0 md:mr-0 lg:ml-0 lg:mr-24 text-blue-500 hover:text-opacity-80 text-sm" href="{{ route('admin.password.request') }}">
                        {{ __('Forgot your password?') }}
                    </a>
                @endif

                <x-primary-button x-cloak x-bind:disabled="buttonDisabled" x-bind:class="buttonDisabled ? 'cursor-not-allowed' : 'cursor-pointer' ">
                    {{ __('Enter') }}
                    <svg
