<div class="w-full px-6 pb-6 h-screen overflow-y-auto">
	...
	<div class="flex-1 min-w-0 py-2">
		<p class="pb-6 pt-3 font-bold uppercase text-sm leading-7 tracking-wider text-gray-600"><span class="flex items-center gap-1"> <x-ri-home-3-fill class="w-4 h-4" /> Home<x-heroicon-o-arrow-long-right class="w-5 h-6" />Maintenance<x-heroicon-o-arrow-long-right class="w-5 h-6" />Program List</span></p>
	</div>
	...
	>
	  <table class="min-w-full border-separate divide-y divide-gray-200 border-b-2 shadow">
		<thead class="bg-gray-50 whitespace-nowrap">
		  ...
		  >
			...
			<th class="font-semibold text-center px-4 text-gray-700 uppercase tracking-normal">College
			...
			<th class="font-semibold text-center px-4 text-gray-700 uppercase tracking-normal">Program
		  ...
		  >
			<td class="px-6 py-6">
			  <div class="flex items-center justify-center">
				<div>
				  <p class="text-md font-medium leading-normal text-gray-800">{{ $curriculum->id }}</p>
				</div>
			  </div>
			</td>
			<td class="px-4 py-6">
			  <p class="text-md font-medium leading-normal text-gray-800">
				{{ $curriculum->dept_description ?? 'Department Not Found' }}
			  </p>
			</td>
			<td class="px-4">
			  <p class="text-md font-medium leading-normal text-gray-800">{{ $curriculum->curr_description }}</p>
			</td>
			<td class="px-4 py-6">
				<p class="text-md font-medium leading-normal text-gray-800">{{ $curriculum->curr_name }}</p>
			  </td>
			<td class="px-4 py-6">
				<p class="text-md font-medium leading-normal text-gray-800">{{ $curriculum->created_at->format('m/d/Y') }}</p>
			</td>
			<td class="px-4 py-6">
			...
			<td class="px-8 py-6 whitespace-nowrap">
				<div class="flex justify-center space-x-2">
					<button wire:loading.attr="disabled" wire:loading.class="cursor-not-allowed" wire:click="view({{ $curriculum->id }})" class="cursor-pointer fa-solid fa-eye text-slate-900 hover:text-opacity-70 duration-150 fa-xl"></button>
					<button wire:loading.attr="disabled" wire:loading.class="cursor-not-allowed" wire:click="edit({{ $curriculum->id }})" class="cursor-pointer fa-solid fa-pen-to-square text-blue-500 hover:text-opacity-70 duration-150 fa-xl"></button>
					<button wire:loading.attr="disabled" wire:loading.class="cursor-not-allowed" wire:click="disable({{ $curriculum->id }})" class="cursor-pointer fa-solid {{ $curriculum->curr_status ? 'fa-user-slash text-red-600' : 'fa-user-check text-green-600' }} hover:text-opacity-70 duration-150 fa-xl"></button>
				</div>
			</td>
		  </tr>
		  @empty
	  ...
	  <x-dialog-modal wire:model.defer="showEditModal">
		...
		<x-slot name="content">
			...
			<x-slot name="footer">
				<x-secondary-button wire:loading.attr="disabled" wire:loading.class="cursor-not-allowed" wire:click="closeModal" class="mx-2">Close</x-secondary-button>
