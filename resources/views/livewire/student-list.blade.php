<div class="w-full px-6 pb-6 h-screen overflow-y-auto">
	...
	<div class="flex-1 min-w-0 py-2">
		<p class="pb-6 pt-3 font-bold uppercase text-sm leading-7 tracking-wider text-gray-600"><span class="flex items-center gap-1"> <x-ri-home-3-fill class="w-4 h-4" /> Home<x-heroicon-o-arrow-long-right class="w-5 h-6" />Menu<x-heroicon-o-arrow-long-right class="w-5 h-6" />Student List</span></p>
	</div>
	...
	>
	  <table class="border-separate min-w-full divide-y divide-gray-200 border-b-2 shadow">
		<thead class="bg-gray-50 whitespace-nowrap">
		  ...
		  >
			...
			<th class="font-semibold text-center px-4 text-gray-700 uppercase tracking-normal">Student Name 
			...
			<th class="font-semibold text-center px-4 text-gray-700 uppercase tracking-normal">Year Level
		  ...
		  >
			...
			<td class="px-8 py-6 whitespace-nowrap">
				<div class="flex justify-center space-x-2">
					<button wire:loading.attr="disabled" wire:loading.class="cursor-not-allowed" wire:click="view({{ $user->id }})" class="cursor-pointer fa-solid fa-eye text-slate-900 hover:text-opacity-70 duration-150 fa-xl"></button>
					<button wire:loading.attr="disabled" wire:loading.class="cursor-not-allowed" wire:click="edit({{ $user->id }})" class="cursor-pointer fa-solid fa-pen-to-square text-blue-600 hover:text-opacity-70 duration-150 fa-xl"></button>
					<button wire:loading.attr="disabled" wire:loading.class="cursor-not-allowed" wire:click="disable({{ $user->id }})" class="cursor-pointer fa-solid {{ $user->acc_status ? 'fa-user-slash text-red-600' : 'fa-user-check text-green-600' }} hover:text-opacity-70 duration-150 fa-xl"></button>
				</div>
			</td>
		  </tr>
		  @empty
		...
		>
		  <td colspan="8" class="px-4">
			<div class="flex items-center justify-center">
			  <div>
				<p class="text-md sm:text-lg py-8 font-medium leading-none text-gray-400">No students found...</p>
			  </div>
			</div>
		  </td>
		</tr>
